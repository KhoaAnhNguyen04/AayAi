// static/pages/configurationsPage.js
import { fetchSourceSyncConfigs, saveSourceSyncConfigs } from "../api.js";
import { renderConfigurationSourceFilter } from "../ui/configurationsUi.js";

let syncConfigs = [];
let refreshIcons = () => {};

export async function initConfigurationPage(options = {}) {
  refreshIcons = options.refreshIcons || refreshIcons;

  try {
    syncConfigs = await fetchSourceSyncConfigs();
  } catch (error) {
    console.error(error);
    syncConfigs = [];
  }
  ...
  exposeConfigurationActions();
  updateConfigurationView();
}

function bindConfigurationEvents() {
  document
    .getElementById("configSearchInput")
    ?.addEventListener("input", updateConfigurationView);

  document
    .getElementById("configFilterSource")
    ?.addEventListener("change", updateConfigurationView);

  document
    .getElementById("configFilterStatus")
    ?.addEventListener("change", updateConfigurationView);
}
...
function updateConfigurationView() {
  ...
  const sourceVal = document.getElementById("configFilterSource")?.value || "";
  const statusVal = document.getElementById("configFilterStatus")?.value || "";

  const processedData = syncConfigs
    .filter((item) => {
      const matchSearch =
        !searchVal ||
        normalizeText(item.source).includes(searchVal) ||
        normalizeText(item.folder).includes(searchVal) ||
        normalizeText(item.source_status).includes(searchVal);

      const matchSource =
        !sourceVal || String(item.source_id) === String(sourceVal);

      const matchStatus =
        !statusVal || String(Boolean(item.sync_enabled)) === statusVal;
  ...
  renderConfigurationSourceFilter(syncConfigs);
  renderConfigurationTable(processedData);
  refreshIcons();
}
...
async function persistConfiguration() {
  const payload = syncConfigs.map((item) => ({
    source_id: item.source_id,
    sync_enabled: Boolean(item.sync_enabled),
  ...
  await saveSourceSyncConfigs(payload);
  syncConfigs = await fetchSourceSyncConfigs();
  updateConfigurationView();
}
...
function exposeConfigurationActions() {
  window.configurationApp = {
    async saveConfig(sourceId) {
      const item = syncConfigs.find(
        (config) => Number(config.source_id) === Number(sourceId),
      );

      if (!item) return;

      const valueInput = document.getElementById(`intervalValue-${sourceId}`);
      const unitInput = document.getElementById(`intervalUnit-${sourceId}`);

      const intervalValue = Number.parseInt(valueInput?.value, 10);
      const intervalUnit = unitInput?.value;

      if (!intervalValue || intervalValue < 1) {
        alert("Khoảng thời gian đồng bộ phải lớn hơn hoặc bằng 1.");
        return;
      }

      if (!["minutes", "hours", "days"].includes(intervalUnit)) {
        alert("Đơn vị thời gian không hợp lệ.");
        return;
      }

      item.interval_value = intervalValue;
      item.interval_unit = intervalUnit;
      item.updated_at = getCurrentTimestamp();

      await persistConfiguration();
    },

    async toggleEnabled(sourceId) {
      const item = syncConfigs.find(
        (config) => Number(config.source_id) === Number(sourceId),
      );
      ...
      item.sync_enabled = !item.sync_enabled;
      item.updated_at = getCurrentTimestamp();
